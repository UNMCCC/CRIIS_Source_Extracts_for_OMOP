...
UNION ALL
SELECT 
	   'MINIVELOS PATIENT_STUDY_STATUS(OMOP_OBSERVATION)' AS IDENTITY_CONTEXT  -- do I get info from Patient or Patient_Study_Status?
	   ,concat_ws("-",src.PKSTUDY_ST,src.PERSONCODE_P,pkPatStudyStat_pps) AS SOURCE_PK  -- INIGO ?????????????????????????????
       ...
       ,'' AS VISIT_DETAIL_ID
       ,status_pss_lu AS OBSERVATION_SOURCE_VALUE   -- INIGO ??????????????????????????????????????????????????????????????????
