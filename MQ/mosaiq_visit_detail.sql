...
SELECT DISTINCT
	  'MOSAIQ MosaiqAdmin Ref_SchSets(OMOP_VISIT_DETAIL)' AS IDENTITY_CONTEXT,
       ...
       isNULL(Ref_SchSets.provider_id,'')			AS PROVIDER_ID,   
	   isNULL(Ref_SchSets.facility_id,'') 			AS CARE_SITE_ID,   -- 5=UNMCC 1201, 51='UNMCC 715', 77='UNMCC SF', 89='UNMMG Lovelace Medical Center OP',102='UNM CRTC II Radiation Oncology'
       isNULL(Ref_SchSets.Activity_desc,'')			AS VISIT_DETAIL_SOURCE_VALUE,  -- -- CHANGE TO ACT DESCRIPTION "Lauer New Patient Appt", "8-hour Chemo"  -- gets translated into CPT after Code Capture
       ''							AS VISIT_DETAIL_SOURCE_CONCEPT_ID,
       ''							AS ADMITTED_FROM_SOURCE_VALUE, -- changed
