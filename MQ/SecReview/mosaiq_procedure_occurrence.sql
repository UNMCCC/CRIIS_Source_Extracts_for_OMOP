...
EXECUTION CHECK SUCCESSFUL -- DAH 01/10/22
...
SELECT distinct  'Mosaiq Ref_SchSet_Charges(OMOP_PROCEDURE_OCCURRENCE)' AS IDENTITY_CONTEXT
            ...
            ,chg.cpt_code			AS PROCEDURE_CONCEPT_ID   -- AND code_type = 0 (from select top 1)
            ,FORMAT(chg.Appt_DtTm,'yyyy-MM-dd HH:mm:ss')		AS PROCEDURE_DATE  -- should this be date only?
			...
			,chg.days_units			AS QUANTITY  -- compare with Adm quantity
			,chg.Chg_Provider_Id	AS PROVIDER_ID
	        ,apptDt_PatID			AS VISIT_OCCURRENCE_ID
	        ,sch_set_id				AS VISIT_DETAIL_ID
			,chg.cpt_code			AS PROCEDURE_SOURCE_VALUE -- AND code_type = 0
