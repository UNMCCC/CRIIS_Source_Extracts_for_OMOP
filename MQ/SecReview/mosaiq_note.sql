...
SELECT 'MOSAIQ NOTES(OMOP_OBSERVATION)' AS IDENTITY_CONTEXT 
	   ...
	   ,FORMAT(NTE.CREATE_DTTM,'yyyy-MM-dd 00:00:00') AS NOTE_DATE
	   ,FORMAT(NTE.CREATE_DTTM,'yyyy-MM-dd HH:mm:ss') AS NOTE_DATETIME
	   ,RTRIM(PRO.TEXT) AS NOTE_TYPE_CONCEPT_ID
	   ,NULL AS NOTE_CLASS_CONCEPT_ID
	   ,case when NTE.SUBJECT <> ' ' 
			THEN RTRIM(NTE.SUBJECT) 
			ELSE RTRIM(PRO.TEXT)
		END NOTE_TITLE
	   ,RTRIM(MosaiqAdmin.dbo.RTF2TXT(NTE.NOTES)) AS NOTE_TEXT
	   ,NULL AS ENCODING_CONCEPT_ID -- 'UTF-8 (32678)' AS ENCODING_CONCEPT_ID
	   ,NULL AS LANGUAGE_CONCEPT_ID -- '4182347' AS LANGUAGE_CONCEPT_ID
	   ,' '  AS PROVIDER_ID
	   ,0 AS VISIT_OCCURRENCE_ID  -- can set to 1st visit of the day
	   ,0 AS VISIT_DETAIL_ID      -- don't set
