...
SELECT 'MosaiqAdmin Ref_Patient_Assessments(OMOP_OBSERVATION)' AS IDENTITY_CONTEXT 
 	   ,rsource.OBX_ID AS SOURCE_PK 
 	   ,rsource.OBX_ID AS OBSERVATION_ID
       ,rsource.pat_id1 AS PERSON_ID
	   ,0	 AS OBSERVATION_CONCEPT_ID
	   ,isNULL(FORMAT(rsource.Obs_DtTm,'yyyy-MM-dd 00:00:00'), '')  AS OBSERVATION_DATE
	   ,isNULL(FORMAT(rsource.Obs_DtTm,'yyyy-MM-dd HH:mm:ss'), '')  AS OBSERVATION_DATETIME
	   ...
	   ,isNULL(rsource.ApptDt_PatID, 0)		AS VISIT_OCCURRENCE_ID
	   ...
	   ,isNULL(FORMAT(rsource.Obs_DtTm,'yyyy-MM-dd HH:mm:ss'), '')  AS VALUE_AS_DATETIME
	   ,isNULL(rsource.obs_desc, '')			AS obs_desc	-- Assessment Question (may be more or less detailed than obs_label)
	   ,isNULL(rsource.obs_label, '')			AS obs_label	-- Assessment Question (may be more or less detailed than obs_label)
	   ,isNULL(rsource.obs_choice_label, '')	AS obs_choice_label -- contains the numeric response if observe.obs_choice is not null --> obs_choice_desc = ObsDef.Description where ObsDef.obd_id = Observe.Obs_choice
	   ,isNULL(rsource.obs_choice_desc, '')		AS obs_choice_desc -- mapped textual response if observe.obs_choice is not null --> obs_choice_desc = ObsDef.Description where ObsDef.obd_id = Observe.Obs_choice
	   ,isNULL(rsource.Obs_Results, '')			AS Obs_Results -- Response as returned by mosaiq function fn_GetObsResult, if obs_choice is not populated
																-- equals obs_choice_label if obs_choice is populated
	   ,isNULL(obs_choice, 0)					AS obs_choice	-- Observe.obd_id that contains response to the assessment, if answer is not in string field
	   ,isNULL(obs_string, '')					AS obs_string	-- Observe.obs_string may contain response if obs_choice is not populated
	   ,isNULL(obs_float, 0)					AS obs_float -- Observe.obs_string may contain response if obs_choice is not populated	 (but a date value will be translated by the function)
