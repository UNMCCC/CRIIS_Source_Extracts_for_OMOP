...
SELECT DISTINCT  -- added DH
	  'MOSAIQ STAFF(OMOP_PROVIDER)' AS IDENTITY_CONTEXT,
       ...
       isNULL(NPI.ID_Code,'') as NPI,
	   isNULL(DEA.ID_Code,'') as DEA,
       ...
       isNULL(rsSource.Staff_ID,'')				AS PROVIDER_SOURCE_VALUE,
	   ...
	   isNULL(Format(rsSource.edit_DtTm,'yyyy-MM-dd HH:mm:ss'),'') as Modified_DtTm	
FROM Mosaiq.dbo.Staff rsSource
LEFT JOIN Mosaiq.dbo.EXT_ID NPI on RsSource.Staff_ID = NPI.Staff_ID and NPI.Ext_Type = 'NPI'
